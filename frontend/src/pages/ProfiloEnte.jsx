...
const ProfiloEnte = () => {
  ...
  const userRole = useMemo(() => profileData?.ruolo?.toLowerCase() || '', [profileData]);
  const isEnte = userRole === 'ente';
  const isVolunteer = userRole === 'volontario';
  const isBeneficiary = userRole === 'beneficiario';
  ...
  const loadData = async () => {
    ...
    } catch (error) { console.error("Errore profilo:", error); return null; }
  };
  ...
  if (!profileData && !currentUser) return <div className="loading-screen">Caricamento profilo...</div>;
  ...
  return (
    <div className="ente-page-wrapper">
          ...
          <div className="controll">
            ...
            <div className="actions-right">
              {isOwner && isEnte ? (
                ...
                <>
                  {isVolunteer && isOwner && <button className="btn-action btn-affiliation" onClick={() => toggleModal('affiliazione', true)}>Richiedi affiliazione</button>}
                  {isBeneficiary && isOwner && <button className="btn-action btn-affiliation" onClick={() => toggleModal('servizio', true)}>Richiedi Servizio</button>}
                </>
              )}
            </div>
          </div>
                ...
                {activeSideTab === 'raccolte' && (
                  <div className="raccolte-list">
                    ...
                    {lists.raccolte.map(r => (
                        <RaccoltaFondiCard key={r.id_raccolta} {...r} isOwner={isOwner && isEnte} onTerminate={handleTerminateRaccolta} />
                    ))}
                  </div>
                )}
              </div>
      ...
      {/* --- MODALI --- */}
      {isOwner && isEnte && modals.addEvento && <AddEvento onBack={() => toggleModal('addEvento', false)} onSubmit={() => { toggleModal('addEvento', false); loadEventi(profileData); }} isModal={true} enteId={profileData?.id} />}
      {isOwner && isEnte && modals.raccolta && <ModalWrapper onClose={() => toggleModal('raccolta', false)}><AddRaccoltaFondi enteLogged={profileData} onClose={() => { toggleModal('raccolta', false); loadRaccolte(profileData); }} isModal={true} /></ModalWrapper>}
      ...
      {isOwner && isEnte && modals.addStory && <AddStory onBack={() => toggleModal('addStory', false)} onSubmit={(d) => handleStoryAction('add', d)} />}
      {isOwner && isEnte && modals.editStory && selectedStory && <EditStory story={selectedStory} onCancel={() => { toggleModal('editStory', false); setSelectedStory(null); }} onSave={(d) => handleStoryAction('edit', d)} />}
      {isOwner && isEnte && modals.deleteStory && selectedStory && <DeleteStory story={selectedStory} onCancel={() => { toggleModal('deleteStory', false); setSelectedStory(null); }} onConfirm={() => handleStoryAction('delete', selectedStory)} />}
    </div>
  );
};
